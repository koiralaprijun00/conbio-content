...
class Consted_Post_Meta {
	...
	public function __construct() {
		...
		add_action( 'consted_do_tags', array( $this, 'render_tags_list' ) );
		add_action( 'consted_site_content_type', array( $this, 'render_meta_list' ), 20, 1 );
		add_action( 'consted_meta_info', array( $this, 'render_meta_list' ), 10, 2 );
	...
	private function get_comments() {
		...
		if ( $comments_number == 0 ) {
			...
			$comments = sprintf( _n( '%s Comment', '%s Comments', $comments_number, 'consted' ), $comments_number );
		}
	...
	public function render_tags_list() {
		...
		$html  = '<div class="other-info"><div class="tags">';
		$html .= '<span>' . esc_html__( 'Tags', 'consted' ) . ':</span>';
		foreach ( $tags as $tag ) {
			$tag_link = get_tag_link( $tag->term_id );
			$html    .= '<i class="icofont-tag"></i><a href=' . esc_url( $tag_link ) . ' title="' . esc_attr( $tag->name ) . '" class=' . esc_attr( $tag->slug ) . ' rel="tag">';
...
$consted_post_meta_class = new Consted_Post_Meta();